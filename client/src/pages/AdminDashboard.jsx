    ...
    <div className="min-h-screen bg-slate-50 px-4 py-10 sm:px-6 lg:px-8">
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <div className="flex flex-wrap gap-3">
          <button onClick={() => setActiveTab('review')} className={`rounded-full px-4 py-2 text-sm font-semibold ${activeTab === 'review' ? 'bg-primary-600 text-white' : 'bg-white text-slate-700 border border-slate-200'}`}>
            Pending Review
            <span className="ml-2 inline-flex min-w-6 items-center justify-center rounded-full bg-white/20 px-2 py-0.5 text-xs font-semibold">{pendingCount}</span>
          </button>
          <button onClick={() => setActiveTab('notifications')} className={`rounded-full px-4 py-2 text-sm font-semibold ${activeTab === 'notifications' ? 'bg-primary-600 text-white' : 'bg-white text-slate-700 border border-slate-200'}`}>
            Notifications
            {unreadCount > 0 ? <span className="ml-2 inline-flex min-w-6 items-center justify-center rounded-full bg-white/20 px-2 py-0.5 text-xs font-semibold">{unreadCount}</span> : null}
          </button>
          <button onClick={() => setActiveTab('users')} className={`rounded-full px-4 py-2 text-sm font-semibold ${activeTab === 'users' ? 'bg-primary-600 text-white' : 'bg-white text-slate-700 border border-slate-200'}`}>Users</button>
        </div>
                  ...
                  <div key={provider._id} className="rounded-xl border border-slate-200 p-4">
                    <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
                      <div className="flex-1 text-left">
                        <button type="button" onClick={() => setExpandedProviderId(isExpanded ? null : provider._id)} className="text-left">
                      ...
                      <div className="flex flex-wrap gap-2">
                        <button onClick={() => navigate(`/provider/${provider._id}`)} className="rounded-lg border border-slate-200 px-3 py-2 text-sm font-semibold text-slate-700">View details</button>
                    ...
                    {isExpanded && (
                      <div className="mt-4 space-y-4 border-t border-slate-100 pt-4 text-sm text-slate-700">
                        <div><div className="font-semibold text-slate-900">Bio</div><div className="mt-1">{provider.bio || 'No bio provided.'}</div></div>
                        <div><div className="font-semibold text-slate-900">Price</div><div className="mt-1">৳{provider.pricePerHour || 'N/A'} / hour</div></div>
                        <div>
                          ...
                          <div className="mt-2 space-y-2">
                            {provider.qualifications?.length > 0 ? provider.qualifications.map((item, index) => (
                              <div key={`${item.qualification || item}-${index}`} className="rounded-lg border border-slate-200 p-2">
                                ...
                                {(item.institution || item.year) && <div className="text-xs text-slate-500 mt-1">{item.institution}{item.institution && item.year ? ' • ' : ''}{item.year}</div>}
                              </div>
                          ...
                          <div className="mt-2 space-y-2">
                            {provider.certifications?.length > 0 ? provider.certifications.map((item, index) => (
                              <div key={`${item.name || item}-${index}`} className="rounded-lg border border-slate-200 p-2">
                                ...
                                {(item.link || item.fileName) && <div className="text-xs text-slate-500 mt-1">{item.link ? `Link: ${item.link}` : `File: ${item.fileName || 'Uploaded document'}`}</div>}
                              </div>
                            )) : <div className="text-slate-500">No certifications added.</div>}
                          </div>
                        </div>
                        <div><div className="font-semibold text-slate-900">Experience</div><div className="mt-1">{provider.experience || 'No experience provided.'}</div></div>
                        <div>
                        ...
                        <div>
                          ...
                          <textarea value={reasonDrafts[provider._id] || ''} onChange={(e) => setReasonDrafts((prev) => ({ ...prev, [provider._id]: e.target.value }))} className="w-full rounded-lg border border-slate-300 p-2 text-sm" rows="2" placeholder="Explain why the profile was rejected" />
          ...
          <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
            ...
            <div className="mt-4 space-y-3">
              {notifications.length === 0 ? (
                ...
                <button key={item._id} onClick={() => handleOpenNotification(item)} className={`w-full rounded-lg border p-3 text-left ${item.isRead ? 'border-slate-200 bg-white' : 'border-primary-200 bg-primary-50'}`}>
                  <div className="flex items-center justify-between gap-3">
                    <div>
                      <div className="font-semibold text-slate-800">{item.title}</div>
                      <div className="text-sm text-slate-600 mt-1">{item.message}</div>
                    </div>
                    {!item.isRead && <span className="rounded-full bg-primary-600 px-2 py-1 text-xs font-semibold text-white">New</span>}
                  </div>
                </button>
              ))}
            </div>
